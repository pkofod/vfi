    ...
    def evaluate_(self,xi):
        ...
        for i in range(self.ncube):
            ...
            for l in range(self.dimy):
                ...
                self.yi_temp[l] += self.y[l*self.Ny + self.indexes[i] + index0]*(self.weights[i*2+0]*(1.0-self.reldiff[self.dimx-1]))
                self.yi_temp[l] += self.y[l*self.Ny + self.indexes[i] + index0+1]*(self.weights[i*2+1]*self.reldiff[self.dimx-1])
